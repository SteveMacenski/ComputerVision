...
leg{nl} = sprintf('Ground Truth: m=%0.2f b=%0.2f', gt_m, gt_b);
...
%% Create points used to estimate line
...
% find line that best predicts y given x
% minimize sum_i (m*x_i + b - y_i).^2
thetas = (-pi+pi/50):(pi/100):pi; % 100 bins
...
r = minr + stepr*(rind-1);
figure(3), hold off, imagesc(counts'), axis image, colormap gray
...
figure(4), hold off, plot(x, y, 'r*'), hold on, plot(x(inliers), y(inliers), 'g*')
